# Caller — continuar chat Codex existente

**Asignación vigente 2026-09-17:** NBC 01 | AI Caller; tarea C08. Modelo inicial: Sol / alto para contrato inicial; después Terra / medio. Leer `docs/lanes/tasks/OR04-start-2026-09-17.md`, que sustituye propuestas OR03 y datos de acceso históricos del bloque inferior. El arranque explícito de Franco habilita ejecutar el encargo, sin iniciar campañas por inferencia.
...
Seguís como lane Caller, conservando contexto C01–C07. No reconstruyas voz de navegador, CRM ni listas.

MODELOS: Terra/medio de base. Luna/bajo para inventario/documentación o cambios mecánicos. Sol/alto para contratos de telefonía, idempotencia, concurrencia, webhooks y límites de gasto. Astra/alto solo por bloqueo difícil, con motivo y alcance acotado; después pedir volver a Terra. Usar el selector local como ayuda, sin fingir que cambia el IDE.

FOCO: C08 propuesto en docs/lanes/tasks/OR03-effective-first.md: llamada telefónica a número propio, estados/resultado persistentes, recuperación y Stop, DNC y límites. Clonar voz no bloquea el piloto; una voz existente sirve. Test de navegador/cola demo no son telefonía Live. Prepará implementación independiente mientras falten proveedor/número; no comprar ni llamar leads por inferencia. Sin nuevos rediseños.

OWNERSHIP: componentes Caller y familias caller-*, servicios/rutas/tests propios. No editar shell, auth común, dependencias ni archivos ajenos. C-RECOVERY sigue pendiente según revisión; L02/T1 locales de otros módulos se preservan. Contrastar fuente antes de corregir.

ENTREGA DEL ENCARGO: reporte C08 cuando se asigne ese ID, sin sobrescribir C01–C07. Incluir qué parte permite una llamada real y qué solo está probada con fixtures.
