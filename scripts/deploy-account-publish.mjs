...
const current=await currentResponse.json();if(!currentResponse.ok||current.deployment?.id!=='dpl_54DpwEfF8cNcbHJi4HmieufQESio')throw Error('Alias changed: reconcile latest verified source before publishing.');
const r=await fetch(`https://api.vercel.com/v13/deployments?teamId=${process.env.VERCEL_TEAM_ID}`,{method:'POST',headers:{Authorization:`Bearer ${process.env.VERCEL_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({name:'nbc-sales',project:process.env.VERCEL_PROJECT_ID,target:'staging',files,meta:{overviewCandidate:'I06-R1',sessionCandidate:'S01-R1-plus-I05',memberCandidate:'I02-R4',callerCandidate:'C06-R2',academyCandidate:'K01-R6',leadEngineCandidate:'L01-R5',baseDeployment:'dpl_54DpwEfF8cNcbHJi4HmieufQESio',accountUsageCandidate:'I06-R1',overviewManifest:hash}}),signal:AbortSignal.timeout(60000)});
