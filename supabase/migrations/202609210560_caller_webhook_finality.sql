...
begin
  ...
  update public.call_sessions set
    -- Once exposed as final, a delayed or changed delivery may enrich it but cannot flip the outcome.
    status=case when target.status in ('completed','failed') then target.status else p_status end,
    ...
    transcript=case
      when jsonb_array_length(p_transcript) > jsonb_array_length(transcript) then p_transcript
      when jsonb_array_length(p_transcript) = jsonb_array_length(transcript) and char_length(p_transcript::text) >= char_length(transcript::text) then p_transcript
      else transcript
    end,
    summary=coalesce(p_summary,summary),
    post_call_analysis=coalesce(p_analysis,post_call_analysis),
    failure_code=case
      when target.status='failed' then target.failure_code
      when target.status='completed' then null
      when p_status='failed' then coalesce(p_failure,failure_code,'provider_failed')
      else null
    end,
